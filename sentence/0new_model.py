# ------------------------      wait to train   ----------------------------------
# pm2 start  file name -- interpreter env/bin/python3
...
def load_data(filepath):
    ...
    print("MAX...",max(len(padded_labels[i]) for i in range(len(padded_labels))))
    # print(padded_labels)
    cropped_l = []
    for i in valid_images:
        ...
        elif len(i.shape) == 3 and i.shape[2] == 1:
            i = i[:, :, 0]  
        ...
        thresh = cv2.adaptiveThreshold(i, 255, cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY_INV, 35, 10)
...
train_image,train_label = load_data("../model/new_ocr_train.npy")
valid_image,valid_label = load_data("../model/new_ocr_valid.npy")
# test_image,test_label = load_data("../model/new_ocr_test.npy")
        ...
        for i in range(0, num_samples, batch_size):
            ...
            for img, lbl in zip(batch_images, batch_labels):
                ...
                if len(actual_label) > 0:
                    ...
                    if img.ndim == 2:
                        img = np.expand_dims(img, axis=-1)  # (64, 256) → (64, 256, 1)
                    valid_images.append(img)
                    valid_labels.append(lbl)
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint('clean_best_model.keras', save_best_only=True),
...
model.save("clean_data_model.keras")
